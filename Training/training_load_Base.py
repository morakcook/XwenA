...
def training(model_name, plus_epoch_model, load_dataset_path, epochs, steps, checkpoint):
    # 데이터셋 로드
...
# 이전에 fine-tuning(미세 조정)을 거친 모델
checkpoint = "/home/dacon_project/CheckPoints/Test_5epoch_Qlora"

# fine-tuning(미세 조정)을 거친 후의 모델에 부여될 새로운 이름을 지정하는 변수
plus_epoch_model = "/home/dacon_project/CheckPoints/Test_10epoch_Qlora"

# 데이터셋 경로
load_dataset_path = '/home/dacon_project/Data/Load_data/Base_Q_A'

# 사용할 파라미터 설정
epochs = 5
steps = 25

training(model_name, plus_epoch_model, load_dataset_path, epochs, steps, checkpoint)